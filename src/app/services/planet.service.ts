...
export class PlanetService {
  ...
  getAllPlanets(): Planet[] {
    return this.planets; // Fonction qui va retourner toutes nos planètes
  }

  getOnePlanetById(id: number): Planet {
    return this.planets.filter(planet => planet.id === id)[0]; //retourne une seule planete selon l'ID

  }

  addPlanet(planet: Planet): void {
    this.planets.push(planet) //on va ajouter (push) dans le tableau une planete
  }
